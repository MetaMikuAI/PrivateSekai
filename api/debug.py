...
@debug_bp.route('/metamiku/debug/getUserNotSuite/<int:user_id>', methods=['GET'])
def handle_get_user_not_suite(user_id: int):
    if not DEBUG:
        return Response("Debug mode is off", status=403)
    ...
    user = users.get_user(user_id)
    response_data = user.get_all_data()
    response_body = json.dumps(response_data, ensure_ascii=False)
    return Response(response_body, content_type='application/json')

@debug_bp.route('/metamiku/debug/getUserAllData/<int:user_id>', methods=['GET'])
def handle_get_user_all_data(user_id: int):
    if not DEBUG:
        return Response("Debug mode is off", status=403)
    ...
    user = users.get_user(user_id)
    response_data = {
        "suite": user.get_suite_user_data(),
        "not_suite": user.get_not_suite_data()
    }
    response_body = json.dumps(response_data, ensure_ascii=False)
    return Response(response_body, content_type='application/json')